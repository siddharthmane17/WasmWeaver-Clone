...
class AbstractLocalFactory(AbstractTileFactory):
    ...
    def generate_all_placeable_tiles(self, global_state: GlobalState, current_function: Function) -> [
        ...
        for index, local in enumerate(global_state.stack.get_current_frame().locals.locals):
            ...
            if local_get_tile.can_be_placed(global_state, current_function):
                yield local_get_tile
            local_set_tile = self.create_local_set_tee_tile(index)
            if local_set_tile.can_be_placed(global_state, current_function):
        ...
        if new_tee_tile.can_be_placed(global_state, current_function):
            yield new_tee_tile
